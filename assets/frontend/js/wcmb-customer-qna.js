    ...
    $('#cust_question').on('keyup', function () {
   	...
   	keyup_timeout = setTimeout(function(){
            ...
            if(this_ele_val.length > 3){
                var data = {
                    action: 'wcmb_customer_ask_qna_handler',
                    handler: 'search',
                    product_ID: $('#product_ID').val(),
                    keyword: this_ele_val
                };
                $.post(wcmb_customer_qna_js_script_data.ajax_url, data, function (response) { 
...
//                },3000);
    ...
    $('body').on('click', 'button.wcmb-add-qna-reply', function () { 
...
//                setTimeout($('#qna-reply-modal-'+key).modal('hide'),3000);
    ...
    $('body').on('click', 'button.wcmb-update-qna-answer', function () { 
        var key = $(this).attr('data-key');
