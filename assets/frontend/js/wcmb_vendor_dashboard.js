/*global wcmb_frontend_vdashboard_js_script_data */
jQuery(document).ready(function($){
	$("body").on("click", ".wcmb_delate_announcements_dashboard", function(e) {
			...
			lodder.show();			
			var data = {
				action : 'wcmb_dismiss_dashboard_announcements',
				post_id : post_id
			}
			$.post(wcmb_frontend_vdashboard_js_script_data.ajax_url, data, function(responsee) { 
	...
	$(".wcmb_frontend_sale_show_more_button").click(function(e) {
		...
		if( button_type == 'sale_weekly_more') {
			...
			myaction = 'wcmb_frontend_sale_get_row';
			tobeappend = 'wcmb_sale_report_table_week';
			first_number = 'wcmb_front_count_first_num_week';
			...
			myaction = 'wcmb_frontend_sale_get_row';
			tobeappend = 'wcmb_sale_report_table_today';
			first_number = 'wcmb_front_count_first_num_today';
	...
	$(".wcmb_frontend_pending_shipping_show_more_button").click(function(e) {
		var lodder_parent = $(this).parent();	
		var ajax_loader_class = $(lodder_parent).find('.ajax_loader_class');		
		$(ajax_loader_class).show();		
		var current_page = '';
		...
		if( button_type == 'pending_shipping_weekly_more') {
			...
			today_or_weekly = 'weekly';
			myaction = 'wcmb_frontend_pending_shipping_get_row';
			tobeappend = 'wcmb_pending_shipping_report_table_week';
			first_number = 'wcmb_front_count_first_num_week_ps';
			...
			today_or_weekly = 'today';
			myaction = 'wcmb_frontend_pending_shipping_get_row';
			tobeappend = 'wcmb_pending_shipping_report_table_today';
			first_number = 'wcmb_front_count_first_num_today_ps';
        ...
        $("#wcmb_vendor_stats_report_filter").change(function (e) {
            ...
            $.each(current_data, function (key, value) {
                if(key == '_wcmb_stats_table'){
                    $.each(current_data._wcmb_stats_table, function (subkey, subvalue) {
                        $('.'+key+'.'+subkey).html(subvalue);
                        if(subkey == 'current_withdrawal' && current_data._raw_stats_data.current.withdrawal == 0){
                            $('.'+key+'.'+subkey+'.withdrawal-label').html(current_data._wcmb_stats_lang_no_amount);
                        }
                    });
                    ...
                    $.each(current_data.stats_difference, function (subkey, subvalue) {
                        ...
                        if(subkey == '_wcmb_diff_orders_no'){
                            if(subvalue > 0){
                                $('.'+subkey).html(current_data._wcmb_stats_lang_are_up+' '+Math.abs(subvalue));
                                $('.'+subkey).addClass('mark-green');
                            }else if(subvalue == 0){
                                $('.'+subkey).html(current_data._wcmb_stats_lang_same);
                                $('.'+subkey).addClass('mark-green');
                            }else if(subvalue == 'no_data'){
                                $('.'+subkey).html(current_data._wcmb_stats_lang_no_prev);
                                $('.'+subkey).addClass('mark-green');
                            }else{
                                $('.'+subkey).html(current_data._wcmb_stats_lang_are_down+' '+Math.abs(subvalue));
                                $('.'+subkey).addClass('mark-red');
                            }
                        }else{
                            if(subvalue > 0){
                                $('.'+subkey).html(current_data._wcmb_stats_lang_up+' '+Math.abs(subvalue)+'%');
                                $('.'+subkey).addClass('mark-green');
                            }else if(subvalue == 0){
                                $('.'+subkey).html(current_data._wcmb_stats_lang_same);
                                $('.'+subkey).addClass('mark-green');
                                ...
                                $('.'+subkey).html(current_data._wcmb_stats_lang_down+' '+Math.abs(subvalue)+'%');
                                $('.'+subkey).addClass('mark-red');
                            }
                        }
                    });
    ...
    $('#wcmb-change-pass').on('click', function(){
        $(this).parents('.wcmb-do-change-pass').toggle();
