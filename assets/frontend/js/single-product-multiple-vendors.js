jQuery(document).ready(function ($) { 
    ...
    if (parseInt(total_div) > 4) {
        ...
        $.post(wcmb_single_product_multiple_vendors_script_data.ajax_url, data, function (response) {
            $(SingleProductMultivendor).append(response);
        });
    }
    $('body').on('click', 'button#wcmb-load-more-button', function (e) {
        $(SingleProductMultivendor+" .rowbody").each(function () {
            $(this).show('slow');
        });
        $(this).hide('slow');
    });
        $('#wcmb_multiple_product_sorting').change(function (e) { 
            ...
            if (sorting_value != '') {
                ...
                $.post(wcmb_single_product_multiple_vendors_script_data.ajax_url, sorting_data, function (response) {
                    $(SingleProductMultivendor+ ' .rowbody').each(function () {
                        $(this).remove();
                    });
                    ...
                    if (parseInt(total_div2) > 4) {
                        if ($(SingleProductMultivendor+ ' #wcmb-load-more-button').css('display') != 'none') {
                            $(SingleProductMultivendor+ " .rowbody").each(function () {
