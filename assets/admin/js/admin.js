jQuery(document).ready(function ($) {
    ...
    $( '.wcmb-shipping-zone-method' ).on( 'change', '.wcmb-shipping-zone-method-enabled input', function() {
            if ( $( this ).is( ':checked' ) ) {
                    $( this ).closest( '.wcmb-input-toggle' ).removeClass( 'woocommerce-input-toggle--disabled' );
                    $( this ).closest( '.wcmb-input-toggle' ).addClass( 'checked' );
                    $( this ).closest( '.wcmb-input-toggle' ).find( '.wcmb-input-toggle' ).removeClass( 'hide' );
            } else {
                    $( this ).closest( '.wcmb-input-toggle' ).addClass( 'woocommerce-input-toggle--disabled' );
                    $( this ).closest( '.wcmb-input-toggle' ).removeClass( 'checked' );
                    $( this ).closest( '.wcmb-input-toggle' ).find( '.wcmb-shipping-zone-method' ).addClass( 'hide' );
            }
    } );

    $( '.wcmb-shipping-zone-method' ).on( 'click', '.wcmb-shipping-zone-method-enabled', function( e ) {
    ...
    if ($('#wcmb_disbursal_mode_admin').is(':checked')) {
    ...
    if ($('#wcmb_disbursal_mode_vendor').is(':checked')) {
    ...
    $('#wcmb_disbursal_mode_vendor').change(function () {
        if ($(this).is(':checked')) {
            $('#commission_transfer').closest("tr").show();
            $('#no_of_orders').closest("tr").show();
        } else {
    ...
    $( "input[name^='wcmb_payment_settings_name[gateway_charge_fixed_with_']" ).closest('tr').hide();
            ...
            $('.automatic_payment_method').each(function(){
                ...
                if (id !== undefined) {
                    ...
                    if($(this).is(':checked') && $('#payment_gateway_charge').prop('checked')){
                        ...
                        $('#' + terget_id).attr('placeholder', wcmb_admin_js_script_data.lang.in_percentage);
                        $('#' + terget_id).siblings('.description').html($('#' + terget_id).siblings('.description').html()+' '+wcmb_admin_js_script_data.lang.in_percentage);
                        $('#' + terget_fixed_id).closest('tr').show();
                        $('#' + terget_fixed_id).attr('placeholder', wcmb_admin_js_script_data.lang.in_fixed);
                        $('#' + terget_fixed_id).siblings('.description').html($('#' + terget_fixed_id).siblings('.description').html()+' '+wcmb_admin_js_script_data.lang.in_fixed);
                    }else{
	...
	$('.vendor-preview').click(function() {
		...
		if ( $previewButton.data( 'vendor-data' ) ) {
			$( this ).WCBackboneModal({
				template: 'wcmb-modal-view-vendor',
			...
			$.ajax({
				url:     wcmb_admin_js_script_data.ajax_url,
				data:    {
					vendor_id: $vendor_id,
					action  : 'wcmb_get_vendor_details',
					nonce: wcmb_admin_js_script_data.vendors_nonce
				},
				type:    'GET',
	...
	$( document.body ).on('click', '#wc-backbone-modal-dialog .wcmb-action-button', function(e) {
        e.preventDefault();
        $('.wcmb-loader').html('<span class="dashicons dashicons-image-rotate"></span>');
		var $actionButton    = $( this ),
			$vendor_id       = $actionButton.data( 'vendor-id' ),
			$vendor_action   = $actionButton.data( 'ajax-action' );
			$pending_vendor_note = $actionButton.closest( '.wcmb-vendor-modal-main' ).find( '.pending-vendor-note' ).val();
			$note_author_id = $actionButton.closest( '.wcmb-vendor-modal-main' ).find( '.pending-vendor-note' ).data( 'note-author-id' );
