...
if (!class_exists('WCMb_WP_Fields')) {

    class WCMb_WP_Fields {
        ...
        public function radio_select_input($field) {
            ...
            if (isset($field['options']) && !empty($field['options'])) {
                foreach ($field['options'] as $key => $value) {
                    $options .= '<label title="' . esc_attr($key) . '" class="wcmb_template_list"><input class="' . esc_attr($field['class']) . '" style="display:none;" type="radio" ' . checked(esc_attr($field['value']), esc_attr($key), false) . ' value="' . esc_attr($key) . '" name="' . esc_attr($field['name']) . '"><span class="dashicons dashicons-unlock"></span><img src="' . $value . '" /></label><br />';
                }
            }
