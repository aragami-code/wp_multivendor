 ...
 * @package wcmb/API
 */
class WCMb_REST_API_Vendors_Controller extends WC_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/(?P<id>[\d]+)', array(
			'args' => array(
				'id' => array(
					'description' => __( 'Unique identifier for the resource.', 'MB-multivendor' ),
					'type'        => 'integer',
				),
			),
			array(
	...
	public function get_item_permissions_check( $request ) {
		if ( ! current_user_can( 'list_users' ) ) {
			return new WP_Error( 'wcmb_rest_cannot_access', __( 'Sorry, you cannot check list vendors.', 'MB-multivendor' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function create_item_permissions_check( $request ) {
		if ( ! current_user_can( 'create_users' ) ) {
			return new WP_Error( 'wcmb_rest_cannot_create', __( 'Sorry, you cannot create vendors.', 'MB-multivendor' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function update_item_permissions_check( $request ) {
		if ( ! current_user_can( 'edit_users' ) ) {
			return new WP_Error( 'wcmb_rest_cannot_update', __( 'Sorry, you cannot update vendors.', 'MB-multivendor' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function delete_item_permissions_check( $request ) {
		if ( ! current_user_can( 'delete_users' ) ) {
			return new WP_Error( 'wcmb_rest_cannot_delete', __( 'Sorry, you cannot delete vendors.', 'MB-multivendor' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function batch_items_permissions_check( $request ) {
		if ( ! current_user_can( 'edit_users' ) ) {
			return new WP_Error( 'wcmb_rest_cannot_do_batch', __( 'Sorry, you cannot process batch.', 'MB-multivendor' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function get_items( $request ) {
		 ...
		 */
		return apply_filters( "wcmb_rest_prepare_{$this->post_type}_object", $response, $object, $request );
	}
    ...
    public function prepare_item_for_response( $method, $request, $additional_fields = [] ) {
    	...
    	$vendor_object = apply_filters("wcmb_rest_prepare_vendor_object_args", array(
    		'id' => $method->id,
        ...
        return apply_filters( 'wcmb_rest_prepare_{$this->post_type}_method', $response, $method, $request );
    }
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			...
			return new WP_Error( "wcmb_rest_{$this->post_type}_exists", sprintf( __( 'Cannot create existing %s.', 'MB-multivendor' ), $this->post_type ), array( 'status' => 400 ) );
		}
		
		if ( empty( $request['login'] ) ) {
			/* translators: %s: post type */
			return new WP_Error( "wcmb_rest_{$this->post_type}_login_empty", sprintf( __( '%s login required.', 'MB-multivendor' ), $this->post_type ), array( 'status' => 400 ) );
		}
		
		if ( empty( $request['email'] ) ) {
			/* translators: %s: post type */
			return new WP_Error( "wcmb_rest_{$this->post_type}_email_empty", sprintf( __( '%s email required.', 'MB-multivendor' ), $this->post_type ), array( 'status' => 400 ) );
		}
		 ...
		 */
		do_action( "wcmb_rest_insert_{$this->post_type}_object", $user_id, $request, true );

		$request->set_param( 'context', 'edit' );
		$response = $this->prepare_item_for_response( get_wcmb_vendor($user_id), $request );
	...
	public function get_item( $request ) {
		if ( empty( $request['id'] ) || $request['id'] == '' ) {
			/* translators: %s: post type */
			return new WP_Error( "wcmb_rest_{$this->post_type}_insufficient_param", sprintf( __( 'Parameter insufficient for %s.', 'MB-multivendor' ), $this->post_type ), array( 'status' => 400 ) );
		}
		
		if( ! is_user_wcmb_vendor($request['id']) ) {
			/* translators: %s: post type */
			return new WP_Error( "wcmb_rest_is_not_a_{$this->post_type}", sprintf( __( 'User is not a %s.', 'MB-multivendor' ), $this->post_type ), array( 'status' => 400 ) );
		}
		
		$vendor = get_wcmb_vendor($request['id']);
		$vendor_data = $this->prepare_item_for_response( $vendor, $request );
		 ...
		 *
		 * @param WP_REST_Response $response The response object.
		 * @param wcmb_Vendor      $vendor   Vendor object data.
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( "wcmb_rest_prepare_single_{$this->post_type}_object", $response, $vendor, $request );
	}
	...
	public function update_item( $request ) {
		 ...
		 * @param boolean         $creating  True when creating object, false when updating.
		 */
		do_action( "wcmb_rest_insert_{$this->post_type}_object", $user_id, $request, false );
	...
	public function delete_item( $request ) {
		...
		$previous = $this->prepare_item_for_response( get_wcmb_vendor( $request['id'] ), $request );
		
		$deleted = wp_delete_user( $request['id'] ) ;
		
		if ( ! $deleted ) {
			return new WP_Error( 'wcmb_rest_cannot_delete', __( 'The vendor cannot be deleted.' ), array( 'status' => 500 ) );
        }
		 ...
		 * @param WP_REST_Request $request   Request object.
		 */
		do_action( "wcmb_rest_delete_{$this->post_type}_object", $request['id'], $request );
