...
function assign_product_to_vendor($object, $request, $new_product) {
	...
	if(isset($request['vendor']) && $request['vendor'] != '') {
		...
		if(isset($vendor->user_data->roles) && in_array('dc_vendor', $vendor->user_data->roles)) {
			...
			wp_set_object_terms($object->get_id(), absint($vendor->term_id), $WCMb->taxonomy->taxonomy_name);
		} else {
			return new WP_Error(
				"woocommerce_rest_product_invalid_vendor_id", __( 'Invalid Vendor ID.', 'MB-multivendor' ), array(
					'status' => 404,
				)
			);
		}
...
function return_vendor_info_on_list_product_query($response, $object, $request) {
	...
	if(isset($vendor->user_data->roles) && in_array('dc_vendor', $vendor->user_data->roles)) {
		...
		$response->set_data( apply_filters( 'before_wcmb_rest_prepare_product_object_data', $data, $vendor, $object, $request ) );
	}
	return $response;
}
