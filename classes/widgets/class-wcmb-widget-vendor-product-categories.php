...
class WCMb_Widget_Vendor_Product_Categories extends WC_Widget {

    public $vendor_term_id;

    public function __construct() {
        $this->widget_cssclass = 'wcmb woocommerce wcmb_widget_vendor_product_categories widget_product_categories';
        $this->widget_description = __('Displays a list of product categories added by the vendor on the vendor shop page.', 'MB-multivendor');
        $this->widget_id = 'wcmb_vendor_product_categories';
        $this->widget_name = __('MB: Vendor\'s Product Categories', 'MB-multivendor');
        $this->settings = array(
            'title' => array(
                'type' => 'text',
                'std' => __('Vendor Product categories', 'MB-multivendor'),
                'label' => __('Title', 'MB-multivendor'),
            ),
            'count' => array(
                'type' => 'checkbox',
                'std' => 1,
                'label' => __('Show product counts', 'MB-multivendor'),
            ),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => __('Show hierarchy', 'MB-multivendor'),
			),
        );
        parent::__construct();
    }
    ...
    public function widget($args, $instance) {
        ...
        if (!is_tax($WCMb->taxonomy->taxonomy_name) && !$vendor) {
            return;
        }
    ...
    function get_hierarchical_categories($terms, $associated_terms, $show_count, $parent_id = 0) {
    	...
    	foreach ($terms as $term_key => $term) {
            if(apply_filters('wcmb_widget_show_vpc_hierarchical_if_no_parents', true)){
