 ...
 */
class WCMb_Transaction {

    private $post_type;

    public function __construct() {
        $this->post_type = 'wcmb_transaction';
    ...
    private function register_post_type() {
        ...
        $labels = array(
            'name' => _x('Transactions', 'post type general name', 'MB-multivendor'),
            'singular_name' => _x('Transaction', 'post type singular name', 'MB-multivendor'),
            'add_new' => _x('Add New', $this->post_type, 'MB-multivendor'),
            'add_new_item' => sprintf(__('Add New %s', 'MB-multivendor'), __('Transaction', 'MB-multivendor')),
            'edit_item' => sprintf(__('Edit %s', 'MB-multivendor'), __('Transaction', 'MB-multivendor')),
            'new_item' => sprintf(__('New %s', 'MB-multivendor'), __('Transaction', 'MB-multivendor')),
            'all_items' => sprintf(__('All %s', 'MB-multivendor'), __('Transaction', 'MB-multivendor')),
            'view_item' => sprintf(__('View %s', 'MB-multivendor'), __('Transaction', 'MB-multivendor')),
            'search_items' => sprintf(__('Search %a', 'MB-multivendor'), __('Transactions', 'MB-multivendor')),
            'not_found' => sprintf(__('No %s found', 'MB-multivendor'), __('Transactions', 'MB-multivendor')),
            'not_found_in_trash' => sprintf(__('No %s found In trash', 'MB-multivendor'), __('Transactions', 'MB-multivendor')),
            'parent_item_colon' => '',
            'all_items' => __('Transactions', 'MB-multivendor'),
            'menu_name' => __('Transactions', 'MB-multivendor')
        );
    ...
    private function register_post_status() {
        register_post_status('wcmb_processing', array(
        ...
        register_post_status('wcmb_completed', array(
        ...
        register_post_status('wcmb_canceled', array(
        ...
        if (!empty($transaction_data)) {
            foreach ($transaction_data as $vendor_id => $transaction_detail) {
                ...
                if ($trans_id) {
                    ...
                    if ($transaction_status != 'wcmb_processing') {
    ...
    public function get_transaction_item_totals($transaction_id, $vendor) {
        ...
        $item_totals['date'] = array('label' => __('Date of request', 'MB-multivendor'), 'value' => wcmb_date(get_post($transaction_id)->post_date));
        $item_totals['amount'] = array('label' => __('Amount', 'MB-multivendor'), 'value' => wc_price($transaction_amount));
        if ($transfer_charge) {
            $item_totals['transfer_fee'] = array('label' => __('Transfer Fee', 'MB-multivendor'), 'value' => wc_price($transfer_charge));
        }
        if($gateway_charge){
            $item_totals['gateway_charge'] = array('label' => __('Gateway Fee', 'MB-multivendor'), 'value' => wc_price($gateway_charge));
        }

        if ($transaction_mode == 'direct_bank') {
            $item_totals['via'] = array('label' => __('Transaction Mode', 'MB-multivendor'), 'value' => __('Direct Bank', 'MB-multivendor'));
            $item_totals['bank_account_type'] = array('label' => __('Bank Account Type', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_bank_account_type', true));
            $item_totals['bank_account_name'] = array('label' => __('Bank Account Number', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_bank_account_number', true));
            $item_totals['bank_name'] = array('label' => __('Bank Name', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_bank_name', true));
            $item_totals['aba_routing_number'] = array('label' => __('ABA Routing Number', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_aba_routing_number', true));
            $item_totals['bank_address'] = array('label' => __('Bank Address', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_bank_address', true));
            $item_totals['destination_currency'] = array('label' => __('Destination Currency', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_destination_currency', true));
            $item_totals['iban'] = array('label' => __('IBAN', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_iban', true));
            $item_totals['account_holder_name'] = array('label' => __('Account Holder Name', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_account_holder_name', true));
        } else if ($transaction_mode == 'paypal_masspay') {
            $item_totals['via'] = array('label' => __('Transaction Mode', 'MB-multivendor'), 'value' => __('PayPal Masspay', 'MB-multivendor'));
            $item_totals['paypal_email'] = array('label' => __('PayPal Email', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_paypal_email', true));
        } else if ($transaction_mode == 'paypal_payout') {
            $item_totals['via'] = array('label' => __('Transaction Mode', 'MB-multivendor'), 'value' => __('PayPal Payout', 'MB-multivendor'));
            $item_totals['paypal_email'] = array('label' => __('PayPal Email', 'MB-multivendor'), 'value' => get_user_meta($vendor->id, '_vendor_paypal_email', true));
        } else if ($transaction_mode == 'stripe_masspay') {
            $item_totals['via'] = array('label' => __('Transaction Mode', 'MB-multivendor'), 'value' => __('Stripe Connect', 'MB-multivendor'));
        } else if ($transaction_mode == 'manual') {
            $item_totals['via'] = array('label' => __('Transaction Mode', 'MB-multivendor'), 'value' => __('Manual', 'MB-multivendor'));
        }
        return apply_filters('wcmb_transaction_item_totals', $item_totals, $transaction_id);
    }
    ...
    public function get_transaction_item_details($transaction_id) {
        ...
        if($status == 'wcmb_completed'){
            $transaction_status = __('Completed', 'MB-multivendor');
        } else if($status == 'wcmb_processing'){
            $transaction_status = __('Processing', 'MB-multivendor');
        }else{
            $transaction_status = __('Cancelled', 'MB-multivendor');
        }
        ...
        $commission_details['header'] = array(__('Commission ID', 'MB-multivendor'), __('Products', 'MB-multivendor'), __('Status', 'MB-multivendor'), __('Amount', 'MB-multivendor'));
        return apply_filters('wcmb_transaction_item_details', $commission_details);
    }
    ...
    public function get_transactions($vendor_term_id = false, $start_date = false, $end_date = false, $transaction_status = false, $offset = false, $no_of = false) {
        ...
        if ($transactions) {
            foreach ($transactions as $transaction_key => $transaction) {
                ...
                if ($transaction->post_status == 'wcmb_completed') {
                    $transaction_details[$transaction->ID]['status'] = __('Completed', 'MB-multivendor');
                } else if ($transaction->post_status == 'wcmb_processing') {
                    $transaction_details[$transaction->ID]['status'] = __('Processing', 'MB-multivendor');
                }
                ...
                if ($mode == 'paypal_masspay') {
                    $transaction_details[$transaction->ID]['mode'] = __('PayPal', 'MB-multivendor');
                } else if ($mode == 'direct_bank') {
                    $transaction_details[$transaction->ID]['mode'] = __('Direct Bank Transfer', 'MB-multivendor');
                }
            }
        }
        return apply_filters('wcmb_get_transaction_details', $transaction_details);
    }
    ...
    public function create_transactions($commission_ids) {
        ...
        if (!empty($commission_ids)) {
            foreach ($commission_ids as $commission_id) {
                ...
                $due_vendor = $vendor->wcmb_get_vendor_part_from_order($order, $vendor_id);
            ...
            $this->insert_new_transaction($transaction_datas, 'wcmb_completed', 'manual');
        }
    }
