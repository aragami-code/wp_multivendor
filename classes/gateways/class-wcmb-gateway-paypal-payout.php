...
class WCMb_Gateway_Paypal_Payout extends WCMb_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->gateway_title = __('Paypal payout', 'MB-multivendor');
        $this->payment_gateway = $this->id;
        $this->enabled = get_wcmb_vendor_settings('payment_method_paypal_payout', 'payment');
        $this->client_id = get_wcmb_vendor_settings('client_id', 'payment', 'paypal_payout');
        $this->client_secret = get_wcmb_vendor_settings('client_secret', 'payment', 'paypal_payout');
        if (get_wcmb_vendor_settings('is_asynchronousmode', 'payment', 'paypal_payout') == 'Enable') {
        ...
        if (get_wcmb_vendor_settings('is_testmode', 'payment', 'paypal_payout') == 'Enable') {
            $this->test_mode = true;
    ...
    public function validate_request() {
        ...
        if ($this->enabled != 'Enable') {
            ...
            $this->message[] = array('message' => __('Paypal payout setting is not configured properly please contact site administrator', 'MB-multivendor'), 'type' => 'error');
            return false;
        } else if (!$this->reciver_email) {
            $this->message[] = array('message' => __('Please update your paypal email to receive commission', 'MB-multivendor'), 'type' => 'error');
            return false;
        }
    ...
    private function generate_access_token() {
        ...
        $response = wp_remote_post( $this->token_endpoint, apply_filters( "wcmb_payment_gateway_{$this->id}_http_process_access_token", array(
                        'timeout' => 60,
                        'headers' => array(
    ...
    private function process_paypal_payout() {
        ...
        $note = sprintf(__('Total commissions earned from %1$s as at %2$s on %3$s', 'MB-multivendor'), get_bloginfo('name'), date('H:i:s'), date('d-m-Y'));
        $request_params = '{
		"sender_batch_header": {
        ...
        $response = wp_remote_post( $this->api_endpoint, apply_filters( "wcmb_payment_gateway_{$this->id}_http_process", array(
                        'timeout' => 60,
                        'headers' => array(
        ...
        if($this->payout_mode == 'true'){
            ...
            if ($batch_status == 'SUCCESS' && $transaction_status == 'SUCCESS') {
                ...
                $this->add_commission_note($this->commissions, __('Payment failed', 'MB-multivendor'));
                return false;
            }
        }else{
            $batch_payout_status = apply_filters('wcmb_paypal_payout_batch_status', array('PENDING', 'PROCESSING', 'SUCCESS', 'NEW'));
