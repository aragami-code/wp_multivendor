	...
	<div class="postbox">
				...
				<li class="custom <?php echo $current_range == 'custom' ? 'active' : ''; ?>">
					...
					<form method="GET">
						<div>
							...
							<input type="submit" class="button" value="<?php esc_attr_e( 'Go', 'MB-multivendor' ); ?>" />
						</div>
					</form>
				</li>
			</ul>
		...
		<div class="left_align pad_left">
			<form name="search_product_form" method="post" action="">
				<p>
					<select id="search_product" name="search_product" class="ajax_chosen_select_products_and_variations" data-placeholder="<?php _e('Search for a product...', 'MB-multivendor') ?>" style="min-width:210px;">
						<?php echo $option; ?>
					</select> 
					<input type="button" style="vertical-align: top;" class="product_report_search" value="<?php _e( 'Show', 'MB-multivendor' ); ?>" />
				</p>
			</form>
		</div>
	</div>
	<div class="postbox sort_chart box_data">
		<?php if( !empty($report_sort_arr) ) { ?>
			<div class="sorting_box">
				<span><b><?php _e( 'Sort By : ', 'MB-multivendor' ); ?></b></span>
				<select name="product_report_sort" class="product_report_sort">
					<option value="total_sales"><?php _e( 'Total Sales', 'MB-multivendor' ); ?></option>
					<option value="admin_earning"><?php _e( 'Total Earnings', 'MB-multivendor' ); ?></option>
				</select>
