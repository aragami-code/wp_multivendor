...
class WCMb_Report_Overview extends WC_Admin_Report {
    ...
    private function query_report_data() {
        ...
        for ($date = $start_date; $date <= strtotime('+1 day', $end_date); $date = strtotime('+1 day', $date)) {
            ...
            $orders = apply_filters('wcmb_filter_orders_report_overview', $qry->get_posts());
            ...
            if (!empty($orders)) {
                foreach ($orders as $order_obj) { 
                    ...
                    if(is_user_wcmb_vendor(get_current_vendor_id()))
                        ...
                        $vendors_orders_amount = get_wcmb_vendor_order_amount(array('order_id' => $order->get_id()));
            ...
            $chart_data_order_count[] = wcmbArrayToObject(array('post_date' => date("Y-m-d H:i:s", $date), 'count' => $order_count));
            $chart_data_sales[] = wcmbArrayToObject(array('post_date' => date("Y-m-d H:i:s", $date), 'sales' => $sales));
            $chart_data_vendor_earnings[] = wcmbArrayToObject(array('post_date' => date("Y-m-d H:i:s", $date), 'vendor_earnings' => $vendor_earnings));
            $chart_data_earnings[] = wcmbArrayToObject(array('post_date' => date("Y-m-d H:i:s", $date), 'earnings' => $earnings));
            $chart_data_avg_sales[] = wcmbArrayToObject(array('post_date' => date("Y-m-d H:i:s", $date), 'avg_sales' => $avg_sales));
        }
    ...
    public function get_chart_legend() {
        ...
        switch ($this->chart_groupby) {
            case 'day' :
                $average_sales_title = sprintf(__('%s Average Daily Sales', 'MB-multivendor'), '<strong>' . wc_price($data->average_sales) . '</strong>');
                break;
            case 'month' :
            default :
                $average_sales_title = sprintf(__('%s Average Monthly Sales', 'MB-multivendor'), '<strong>' . wc_price($data->average_sales) . '</strong>');
                break;
        }

        $legend[] = array(
            'title' => sprintf(__('%s Gross Sales in this Period', 'MB-multivendor'), '<strong>' . wc_price($data->total_sales) . '</strong>'),
            'placeholder' => __('This is the sum of the order totals', 'MB-multivendor'),
        ...
        $legend[] = array(
            'title' => sprintf(__('%s Number of Orders Placed', 'MB-multivendor'), '<strong>' . $data->total_orders . '</strong>'),
            'color' => $this->chart_colours['total_orders'],
            'highlight_series' => 0
        );

        $legend[] = array(
            'title' => sprintf(__('%s Net Earnings in this Period', 'MB-multivendor'), '<strong>' . $data->total_earned . '</strong>'),
        ...
        if (!is_user_wcmb_vendor(get_current_vendor_id())) {
            $legend[] = array(
                'title' => sprintf(__('%s Net Vendor Commission', 'MB-multivendor'), '<strong>' . $data->vendor_total_earned . '</strong>'),
    ...
    public function output_report() {
        global $WCMb;
        $ranges = array(
            'year' => __('Year', 'MB-multivendor'),
            'last_month' => __('Last Month', 'MB-multivendor'),
            'month' => __('This Month', 'MB-multivendor'),
            '7day' => __('Last 7 Days', 'MB-multivendor')
        );
        ...
        include( $WCMb->plugin_path . '/classes/reports/views/html-report-by-date.php');
    }
    ...
    public function get_export_button() {
        ...
        <a
            ...
            class="export_csv"
            data-export="chart"
            data-xaxes="<?php esc_attr_e('Date', 'MB-multivendor'); ?>"
            data-exclude_series="2"
            data-groupby="<?php echo $this->chart_groupby; ?>"
            >
                <?php _e('Export CSV', 'MB-multivendor'); ?>
        </a>
        <?php
    }
    ...
    public function get_main_chart() {
        ...
        <script type="text/javascript">
            ...
            var series = [
            {
            label: "<?php echo esc_js(__('Number of Orders', 'MB-multivendor')) ?>",
            ...
            {
            label: "<?php echo esc_js(__('Total Sales', 'MB-multivendor')) ?>",
        ...
        <?php echo $this->get_currency_tooltip(); ?>
            ...
            {
            label: "<?php echo esc_js(__('Average Order Value', 'MB-multivendor')) ?>",
        ...
        <?php echo $this->get_currency_tooltip(); ?>
            ...
            {
            label: "<?php echo esc_js(__('Total Earnings', 'MB-multivendor')) ?>",
        ...
        <?php if(!is_user_wcmb_vendor(get_current_vendor_id())){ ?>
            {
            label: "<?php echo esc_js(__('Total Earnings by Vendor', 'MB-multivendor')) ?>",
