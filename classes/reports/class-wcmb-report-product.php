...
class WCMb_Report_Product extends WC_Admin_Report {

    /**
     * Output the report
     */
    public function output_report() {
        global $wpdb, $woocommerce, $WCMb;
        ...
        $orders = apply_filters('wcmb_filter_orders_report_product', $qry->get_posts());
        ...
        if (!empty($orders)) {
            ...
            foreach ($orders as $order_obj) {
                ...
                if ($vendor = get_wcmb_vendor(get_current_vendor_id())) {
                    $vendors_orders = get_wcmb_vendor_orders(array('order_id' => $order->get_id(), 'vendor_id' => get_current_vendor_id()));
                } else {
                ...
                foreach ($vendors_orders as $key => $v_order) {
                    try {
                        ...
                        $vendors_orders_amount = get_wcmb_vendor_order_amount(array('order_id' => $order->get_id(), 'product_id' => $v_order->product_id));
            ...
            wp_localize_script('wcmb_report_js', 'wcmb_report_product', array('product_report' => $product_report,
                'report_bk' => $report_bk,
            ...
            $report_html = '<tr><td colspan="3">' . __('Your store has no products.', 'MB-multivendor') . '</td></tr>';
        }

        include( $WCMb->plugin_path . '/classes/reports/views/html-wcmb-report-by-product.php');
    }
