<?php
class WCMB_Vendor_Shipping_Method extends WC_Shipping_Method {
    public $default_option;

    public function __clone() {
        _doing_it_wrong( __FUNCTION__, __( 'Cloning this class could cause catastrophic disasters!', 'MB-multivendor' ), '3.0' );
    }

    public function __wakeup() {
        _doing_it_wrong( __FUNCTION__, __( 'Unserializing is forbidden!', 'MB-multivendor' ), '3.0' );
    }

    public function __construct( $instance_id = 0 ) {
        $this->id                   = 'wcmb_vendor_shipping';
        $this->instance_id          = absint( $instance_id );
        $this->method_title         = __( 'Vendor Shipping', 'MB-multivendor' );
        $this->method_description   = __( 'Charge varying rates based on user defined conditions', 'MB-multivendor' );
    ...
    */
    public function init() {
        $this->instance_form_fields = array(
            'title' => array(
                'title'         => __( 'Method title', 'MB-multivendor' ),
                'type'          => 'text',
                'description'   => __( 'This controls the title which the user sees during checkout.', 'MB-multivendor' ),
                'default'       => __( 'Vendor Shipping', 'MB-multivendor' ),
                'desc_tip'      => true,
            ),
            'tax_status' => array(
                'title'         => __( 'Tax status', 'MB-multivendor' ),
                'type'          => 'select',
                'class'         => 'wc-enhanced-select',
                'default'       => 'taxable',
                'options'       => array(
                    'taxable'   => __( 'Taxable', 'MB-multivendor' ),
                    'none'      => _x( 'None', 'Tax status', 'MB-multivendor' ),
                ),
            )
        );
    ...
    */
    public function calculate_shipping( $package = array() ) {
        ...
        $shipping_methods = WCMB_Shipping_Zone::get_shipping_methods( $zone->get_id(), $vendor_id );
        /*
        * Contributing Author: Vivek Athalye @vnathalye
        */
        $shipping_methods = apply_filters('wcmb_get_shipping_methods_for_shipping_address', $shipping_methods, $package );
        ...
        if( is_array( $rates ) && count( $rates ) > 0 ) {
            ...
            foreach( $rates as $key => $rate ) {
                $this->add_rate( array(
                    'id'        => $rate['id'],
                    'label'     => apply_filters( 'wcmb_vendor_shipping_rate_label', $rate['label'], $rate ),
                    'cost'      => $rate['cost'],
    ...
    */
    public static function free_shipping_is_available( $package, $method ) {
        ...
        return apply_filters( 'wcmb_shipping_free_shipping_is_available', $has_met_min_amount, $package, $method );
    }
    ...
    */
    public function is_available( $package ) {
        ...
        $locations = WCMB_Shipping_Zone::get_locations( $zone->get_id(), $vendor_id );
    ...
    */
    public function get_method_rate_id( $method ) {
        return apply_filters( 'wcmb_get_vendor_shipping_method_id', $method['id'] . ':' . $method['instance_id'] );
    }

    /* @not used */
    public static function is_shipping_enabled_for_seller( $vendor_id ) {
        $vendor_shipping_details = get_user_meta( $vendor_id, '_wcmb_shipping', true );
        if( !empty($vendor_shipping_details) ) {
            $enabled = $vendor_shipping_details['_wcmb_user_shipping_enable'];
            $type = $vendor_shipping_details['_wcmb_user_shipping_type'];
